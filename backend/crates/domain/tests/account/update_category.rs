...
fn test_update_category() -> anyhow::Result<()> {
    ...
    match &events[0] {
        ...
        event => anyhow::bail!("Expected CategoryUpdated event, got {:?}", event),
    }
...
#[test]
fn test_update_category_not_found() -> anyhow::Result<()> {
...
#[test]
fn test_update_category_already_deleted() -> anyhow::Result<()> {
    let (mut account, _, _, category_id) = helper::create_account_with_category()?;
...
#[test]
fn test_update_category_empty_name() -> anyhow::Result<()> {
